...
namespace Blaze3DCreator.GameProject
{
    ...
    {
        private static readonly string _applicationDataPath = $@"{Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData)}\Blaze3DCreator\";
        private static readonly string _projectDataPath;
        private static readonly ObservableCollection<ProjectData> _projects = new ObservableCollection<ProjectData>();
        public static ReadOnlyObservableCollection<ProjectData> Projects { get;}

        //Конструктор чтения данных проекта
        private static void ReadProjectData()
        {
            if(File.Exists(_projectDataPath))
            {
                ...
                foreach (var project in projects)
                {
                    ...
                    {
                        project.Icon = File.ReadAllBytes($@"{project.ProjectPath}\.Blaze\Icon.png");
                        project.Screenshot = File.ReadAllBytes($@"{project.ProjectPath}\.Blaze\Screenshot.png");
